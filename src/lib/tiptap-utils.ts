...
export const handleImageUpload = async (
  ...
  // Validate file
  if (!file) {
    throw new Error("Nie podano pliku."); // Komunikat po polsku
  }

  if (file.size > MAX_FILE_SIZE) {
    throw new Error(
      `Rozmiar pliku przekracza ${MAX_FILE_SIZE / (1024 * 1024)}MB.` // Komunikat po polsku
    );
  }

  // === START: Kod do uploadu do Cloudinary ===
  const CLOUDINARY_CLOUD_NAME = "dryjwcwcc"; // Twój Cloud name
  const CLOUDINARY_UPLOAD_PRESET = "my_unsigned_preset"; // Twój unsigned upload preset
  ...
  return new Promise<string>(async (resolve, reject) => {
    try {
        ...
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            try {
              ...
              resolve(responseData.secure_url); // Zwraca bezpieczny URL obrazka
            } catch (jsonError) {
              console.error("Błąd podczas parsowania odpowiedzi z Cloudinary:", jsonError);
              reject(new Error("Odebrano nieprawidłową odpowiedź z serwera."));
            }
          } else {
            let errorMessage = `Upload do Cloudinary nie powiódł się. Status: ${xhr.status}`;
            try {
              const errorResponse = JSON.parse(xhr.responseText);
              errorMessage = errorResponse.error?.message || errorMessage;
            } catch (e) {
              // Ignorujemy błąd parsowania, jeśli odpowiedź nie jest JSON-em
            }
            console.error("Upload do Cloudinary nie powiódł się:", xhr.status, xhr.responseText);
            reject(new Error(errorMessage + ". Spróbuj ponownie."));
          }
        }
      };

      xhr.onerror = () => {
        console.error("Błąd sieci podczas uploadu do Cloudinary:", xhr);
        reject(new Error("Błąd sieci podczas uploadu. Sprawdź połączenie."));
      };
      ...
      console.error("Ogólny błąd podczas inicjowania uploadu Cloudinary:", error);
      reject(new Error("Wystąpił błąd podczas przygotowania uploadu."));
    }
  });
  // === KONIEC: Kod do uploadu do Cloudinary ===

  // Usunąłem/zakomentowałem stary kod z base64 i placeholderem
}
